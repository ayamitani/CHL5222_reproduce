...
author: "CHL5222 Analysis of Correlated Data"
format: pdf
editor: source
---
...
```{r, echo = FALSE}
dates <- read.csv("data/state-physdist-dates-clean.csv", header = TRUE)
head(dates)
```
...
```{r, echo = FALSE}
cases <- read.csv("data/us-states-clean.csv", header = TRUE)
head(cases)
```
...
Note that we haven't "restricted to days on which a state had at least 30 cumulative cases reported" yet. We will do this after checking the descriptives.
```{r}
finaldata <- cases %>%
  group_by(state) %>%
  # add code
...
Use the `lme` package to fit the linear mixed effects model with change in cases as the outcome as described in the paper and compare the parameter estimates to those in Table 1.
